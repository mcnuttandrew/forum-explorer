...
const generateLabels = (branches, branchModel) => {
  const rootLabel = {
    label: branches.length ? ['conversation', 'root'] : [],
    key: 'root',
  };
  return [rootLabel].concat(
...
const weighTree = tree => {
  ...
  const weight = tree.children.reduce(
    (acc, child) => acc + weighTree(child),
    1,
  );
  tree.weight = weight;
  return weight;
...
function buildScales(range, root, key, {height, width}) {
  ...
  const directions = Object.entries(groupedNodes).reduce(
    (acc, [rowKey, values]) => {
  ...
  const dataDomains = Object.entries(groupedNodes).reduce(
    (acc, [rowKey, values]) => {
      acc[rowKey] = computeDomainForAcessor(
        values,
        d => directions[rowKey](d.x, d.y)[key === 'x' ? 0 : 1],
      );
      return acc;
    },
    {},
...
function generateTreemapLayout(height, width, rootBranch, otherBranches) {
  const rootBranchWeight =
    rootBranch.slice(1).reduce((acc, row) => {
      return (row.weight || 1) + acc;
    }, 0) + rootBranch.length;
  const structuredInput = hierarchy({
    children: [
      {
...
export const forestLayout = {
  preheirarchyManipulation: tree => {
    // drop the stumps
    const stumps = tree.children.filter(
  ...
  layout: ({height, width}, branchModel) => data => {
    ...
    const tempChildren = cloneChildren
      .slice()
      .filter(childWithinThreshold(false));
    ...
    const links = extractLinksFromFlatNodeList(flattenedNodes).filter(
