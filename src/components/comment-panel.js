...
class CommentPanel extends React.Component {
  componentDidUpdate(prevProps) {
    ...
    if (
      hoveredGraphComment &&
      hoveredGraphComment !== prevProps.hoveredGraphComment
    ) {
  ...
  render() {
    ...
    const data = itemsToRender.filter(
      item => item.get('type') !== 'story' || item.get('text'),
    );
    ...
    const viewingRoot = data.every(
      d => d.get('depth') === 0 || d.get('depth') === 1,
    );
    const splitComments =
      isForest && singleComments.size > STUMP_PRUNE_THRESHOLD && viewingRoot;
    ...
    return (
      <div className="overflow-y panel" id="comment-panel" ref="commentPanel">
        ...
        {splitComments && (
          <div className="comment-root-prune-explanation">
            ...
            <h5>
              ...
              <a
                ...
                onClick={d => this.refs.singleComments.scrollIntoView()}
              >
                here.
              </a>
            </h5>
        ...
        {splitComments &&
          data.filter(d => d.get('descendants')).map(buildComment)}
        {splitComments && (
          <div
            ref="singleComments"
