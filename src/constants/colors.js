...
export const NODE_COLOR_UNDER_OPACITY = hexOver(NODE_COLOR, BACKGROUND, OPACITY);
export const COLORS = PAIRS.map(({fill}) => fill);
export const STROKES = PAIRS.map(({stroke}) => stroke);
export const COLORS_UNDER_OPACITY = COLORS.map(color => hexOver(color, BACKGROUND, OPACITY));
