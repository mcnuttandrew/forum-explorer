...
export const setSelectedCommentPathWithGraphComment = buildEasyAction(
  'set-comment-path-with-graph-comment',
);
...
export const toggleCommentSelectionLock = buildEasyAction(
  'toggle-comment-selection-lock',
);
export const unlockAndSearch = buildEasyAction('unlock-and-search');
export const updateGraphPanelDimensions = buildEasyAction(
  'update-graph-panel-dimensions',
);
...
const cleanModels = models =>
  models.map(row => row.map(d => ({...d, term: d.term.split("'")[0]})));

export const sleep = sleepTime => x =>
  new Promise(resolve => setTimeout(() => resolve(x), sleepTime));

export const checkIfTourShouldBeShown = () => dispatch =>
  checkForTour().then(payload =>
    dispatch({type: 'check-if-tour-should-be-shown', payload}),
  );
export const modelData = item => dispatch => {
  dispatchRequest({
    template: SERVER_DEV_MODE
...
export function getAllUsers(dispatch, data) {
  ...
  Promise.all(
    users.map(item => dispatchRequest({template: 'userTemplate', item})),
  ).then(userData =>
    dispatch({
      type: 'get-all-users',
      payload: userData,
...
export const getAllItems = (root, ignoreSettingsUpdate) => dispatch => {
  ...
  const doGeneration = generation =>
    Promise.all(
      generation.map(item => dispatchRequest({template: 'hnTemplate', item})),
    ).then(offspring => {
      dispatch({
      ...
      children = children.concat(
        offspring.map(d => ({children: [], ...d, depth})),
      );
      depth += 1;
      const newgen = offspring.reduce(
        (acc, child) => acc.concat((child && child.kids) || []),
        [],
      );
      if (newgen.length) {
